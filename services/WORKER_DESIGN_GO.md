...
| `tunnel_start` | `executor.go:startPublicTunnel` → `core/ngrok.go` | Partial | Attach worker to target Docker networks and propagate cancellation. |
...
| Lifecycle and signal handling | `main.go:main` | Partial | Publish `stopping`, stop new claims, drain active jobs, then publish `offline`. |
...
| Queue cleanup and post-job recovery | `queue.go:Runner.process/clearActive` | Implemented | Graceful shutdown coordination remains missing. |
...
| Tunnel target discovery | `core/docker_ops.go:PublicTunnelTargets` | Partial | Connect worker to target container networks before using internal IP. |
...
| Highest-priority gaps | Graceful shutdown, full cancellation, realtime listener, tunnel network attachment |
