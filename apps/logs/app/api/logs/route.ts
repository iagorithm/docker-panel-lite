...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.error("app logs query failed", error);
    return NextResponse.json({ error: error instanceof Error ? error.message : "Could not read application logs" }, { status: 500 });
  }
}
